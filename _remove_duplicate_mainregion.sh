...
CHECKIP4="*.*";
if cat myip4.txt | grep -v grep | grep $CHECKIP4 > /dev/null
then
        echo "IP4 Exits." 
        myip4=$(cat myip4.txt)
        echo "IP use= $myip4";
        ...
        echo "$CHECKIP4 not exits, get IP4 and save it to myip4.txt" ;
        curl http://checkip.amazonaws.com > myip4.txt
        echo "IP Address V4 has been added to ipppol/myip4.txt"
